...
1. Kaggle API - Direct download from Kaggle
...
def download_kaggle_dataset():
    ...
    try:
        ...
        print("  Note: This requires Kaggle API credentials (~1.5GB download)")
        
        api = KaggleApi()
        api.authenticate()
        
        # Download the malaria dataset
        api.dataset_download_files('ialopez/sarscov2ct', path='.', unzip=True)
...
if uninfected_dir.exists() and infected_dir.exists():
    ...
    # Priority 2: Try to download from Kaggle
    print("\n2️⃣  Attempting to download from Kaggle...")
    if download_kaggle_dataset():
        ...
        print("  ✗ Could not download from Kaggle")
        
        # Priority 3: Fall back to sample images
        print("\n3️⃣  Falling back to sample images (demo mode)...")
...
if training_source == "samples":
    ...
    print("  Option 1: Run with Kaggle API (if you have credentials)")
    print("  Option 2: Download manually and extract to cell_images/")
    print("     https://www.kaggle.com/datasets/ialopez/sarscov2ct")
